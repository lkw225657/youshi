 ...
 * 个人中心
 */
class MoveUploadController extends MobilebaseController {
	...
	public function move (){
		...
		if ( empty($_SESSION['member']['member_id']) ) {
			header("HTTP/1.0 500 Internal Server Error");
			exit;
		}
		...
		$targetDir = './'.C("UPLOADPATH").'/move/'.$project_no.'/'.$member_info['member_id'].'/'.'upload_tmp';
		$uploadDir = './'.C("UPLOADPATH").'/move/'.$project_no.'/'.$member_info['member_id'].'/';
		...
		die(C("UPLOADPATH").'/move/'.$project_no.'/'.$member_info['member_id'].'/');
	}
