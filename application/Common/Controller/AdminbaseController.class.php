...
class AdminbaseController extends AppframeController {
	 ...
	 * 用户二维码
	 */
	public function geterweima($url,$name)
	{
		//$jiami=base64_encode(base64_encode($id));
		/*echo '<pre>';
		echo $jiemi=base64_decode(base64_decode($jiami));
		echo '<pre>';*/
		//$aa=M('jifen_type');
		//print_r($aa->select());
		vendor('erweima.erweima');//导入类库
		$aa=new \erweima();
		//$url=C("WEB_URL")."/index.php?g=Mobile&m=login&a=register&parent_id=".$id;
		return $filename=$aa->png($url,md5($name).'.jpg','','10');
	...
	public function admin_log($admin_id,$txt)
	{
		$save['admin_id']=$admin_id;
		$save['log']=$txt;
		$log=M('admin_log');
		$save['time']=time();
		// file_put_contents('2222222.txt', $save['log']);
		$log->add($save);
	}
}